...
int objdump_is_ignore_section32(elffile32_t *file, int i)
{
    ...
    if ((uint32_t)(file->shdr[i].sh_type) == SHT_SYMTAB)
        return (1);
    if ((uint32_t)(file->shdr[i].sh_size) == 0)
        return (1);
    if (strcmp((char*)(file->strtable + file->shdr[i].sh_name), ".rel.text")
        == 0)
        return (1);
    if (strcmp((char*)(file->strtable + file->shdr[i].sh_name), ".rel"
        ".eh_frame") == 0)
        return (1);
...
void objdump_display_content_section32(elffile32_t *file, int i)
{
    ...
    for (size_t j = 0; j < file->shdr[i].sh_size; j += 16) {
        ...
        objdump_display_addr32(file, i, j);
        objdump_display_char32(file, i, j);
        printf("\n");
    }
}
...
void objdump_display_section32(elffile32_t *file)
{
    ...
    for (int i = 0; i < file->ehdr->e_shnum; i++) {
        if (!objdump_is_ignore_section32(file, i)) {
            objdump_display_content_section32(file, i);
        }
    }
}