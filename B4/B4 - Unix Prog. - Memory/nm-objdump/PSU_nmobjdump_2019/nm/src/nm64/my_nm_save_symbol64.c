...
unsigned char nm_get_symbol64_2(elffile64_t *file, int i, int local)
{
    ...
    if (file->symtable[i].st_shndx == SHN_UNDEF)
        return ('U');
    if (ELF64_ST_BIND(file->symtable[i].st_info) == STB_GNU_UNIQUE)
        return ('u');
    return ('?');
}
...
{
    int local = (ELF64_ST_BIND(file->symtable[i].st_info) == STB_LOCAL) ? 32
: 0;
    if (ELF64_ST_BIND(file->symtable[i].st_info) == STB_WEAK)
    ...
    if (file->shdr[file->symtable[i].st_shndx].sh_type == SHT_PROGBITS &&
        file->shdr[file->symtable[i].st_shndx].sh_flags == SHF_ALLOC)
        return ('R' + local);
    return (nm_get_symbol64_2(file, i, local));
}
...
void nm_add_symbol64(elffile64_t *file, int i)
{
    ...
    new->st_info = nm_get_symbol64(file, i);
